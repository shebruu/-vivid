...
function MemberManagement({ auth,trip }) {
    ...
    const handleAddMember = () => {
        if (memberLogin) {
            Inertia.post(route('trip.addmember', { tripId: trip.id }), { login: memberLogin });
            setMemberLogin("");  
        }
    };
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Participants </h2>}
        >
            <Navbar2 auth={auth} />
        <div>
            ...
            <ul>
                {trip.users.map(user => (
                    <li key={user.id}>
                        {user.login}
                        <button onClick={() => handleRemoveMember(user.id)}>Retirer</button>
                    </li>
                ))}
            </ul>
            <div>
